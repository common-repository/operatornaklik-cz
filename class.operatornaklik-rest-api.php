...
class Operatornaklik_REST_API {
    ...
    public static function p596979_operatornaklik_get_orders(WP_REST_Request $request)
    {
        ...
        if ($search) {
            ...
            wc_get_order($id) && $results['orders'][] = self::formatOrder(wc_get_order($id));
    ...
    public static function p596979_operatornaklik_get_products(WP_REST_Request $request)
    {
        ...
        if ($id) {
            wc_get_product($id) && $results[] = self::formatProduct(wc_get_product($id));
        ...
        foreach (reset($methods) as $method) {
            if (isset($method->enabled) && $method->enabled === 'yes') {
                $results['shippingMethods'][] = [
                    'name' => $method->title,
                    'trackingUrl' => self::trackingApi($method->title),
//					'all' => json_decode(wp_json_encode($method), true),
                ];
            }
        }
...
//				'all' => $product->get_data(),
        ...
        $resultData = [
            ...
            'price' => $totalPrice,
            'shipping' => $result->get_shipping_method(),
